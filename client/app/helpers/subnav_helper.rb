module SubnavHelper
  ...
  def render_subnav(section, page_or_section, level=1, published_only=nil)
    if RAILS_ENV != 'test' && !section.nil?
      ...
      children = section.slug_is(:news_and_events) ? Section.latest_news(6) : section.all_children(:published_only => published_only, :not_past_events => true)
      ...
      if section.slug_is(:news_and_events)
        html << li_with_active(current_page?(:controller => :sections, :action => :archive, :id => section.id)) do
          link_to("News archive", :controller => :sections, :action => :archive, :id => section.id)
        end
      end
      html << "</ul>"
    end
